...
This is a companion to `ssot.md` (which sets up the SSOT philosophy). This doc
records the *results* of applying that methodology to a concrete corpus of
hand-authored reference files plus an automated PowerPoint-driver tune loop.

## Methodology recap

1. Hand-author reference animations in PowerPoint for Mac's UI, save as
   `.pptx`, extract the `ppt/slides/slide*.xml` timing tree.
2. Feed each candidate shape through the then-current `svg2ooxml` tune loop
   (`svg2ooxml/tools/visual/animation_tune.py`) which opens PowerPoint, enters
...
4. Record the results in the consumer-side SSOT files under the sibling
   `svg2ooxml/src/svg2ooxml/assets/animation_oracle/`.

Historical scratch decks used during the session:

- `svg2ooxml/tmp/example.pptx` — initial compound reference (preset 17 entrance +
  preset 6 grow emph + preset 9 transparency)
- `svg2ooxml/tmp/example3.pptx` — motion path reference
- `svg2ooxml/tmp/example4.pptx` — 4 emphasis effects (preset 35 blink, preset 7/2
  line color, preset 6 text grow, preset 18 underline)
- `svg2ooxml/tmp/example5.pptx` — 5 mainSeq effects plus an interactiveSeq with
  preset 28 compound font-size + color
- `svg2ooxml/tmp/example6.pptx` / `svg2ooxml/.visual_tmp/samples/experiment 6.pptx` —
  preset 19 shape fill color and preset 27 color pulse
- `svg2ooxml/tmp/example7.pptx` / `svg2ooxml/.visual_tmp/samples/experiment 7.pptx` —
  comprehensive 150-animation reference deck covering every
  PowerPoint preset
- `svg2ooxml/.visual_tmp/samples/attrsearch example.pptx` — user-authored
...
Confirmed exhaustive by scanning the historical scratch decks
`svg2ooxml/tmp/example{4,5,6,7}.pptx` and
`svg2ooxml/.visual_tmp/samples/experiment {6,7}.pptx` — 400+ authored animations
...
Codified in the sibling
`svg2ooxml/src/svg2ooxml/assets/animation_oracle/attrname_vocabulary.xml`.
...
Codified in the sibling
`svg2ooxml/src/svg2ooxml/assets/animation_oracle/dead_paths.xml` with
...
All 16 were empirically verified to animate via the historical
`svg2ooxml/.visual_tmp/filter_sweep.py` sweep — each filter produces its signature
...
Codified in the sibling
`svg2ooxml/src/svg2ooxml/assets/animation_oracle/filter_vocabulary.xml`
...
The historical `svg2ooxml/.visual_tmp/xmas_tree_v3_compound.py` script builds an 8-effect
...
The oracle is packaged as a Claude skill in the sibling `svg2ooxml` repo at
`svg2ooxml/.claude/skills/pptx-animation/`, assembled by
`svg2ooxml/tools/build_skill.py`. The skill exposes the oracle through CLI
scripts that Claude can invoke via Bash:
