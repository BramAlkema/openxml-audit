...
Curated oracle material should live under `docs/pptx_oracle/`.
...
Every case should be labeled by one or more tiers from
`openxml_audit.EvidenceTier`:

- `schema-valid`
  Spec-driven or hand-authored XML; PowerPoint provenance not claimed.
- `loadable`
  PowerPoint opens the file without repair.
- `roundtrip-preserved`
  Re-save does not materially rewrite the intended structure.
- `slideshow-verified`
  Slideshow behavior matches intent.
- `ui-authored`
  Authored or re-saved by PowerPoint.

Emitter templates should prefer `ui-authored` or `roundtrip-preserved` cases
whenever possible.
...
2. Save the deck.
3. Extract with `openxml-audit-pptx-lab oracle`.
